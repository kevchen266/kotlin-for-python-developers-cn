...
由于 Kotlin 继承了 Java 的不良设计决策，因此字节数为 -128 至 127。为了获得介于 0 与 255 之间的传统字节值，如果该值是正数，那么将其保持原样；如果它是负数，那么将其加上 256（因此，-128 实际上是 128，而 -1 是真正的 255）。请参见[扩展函数](extension-functionsproperties.html)部分，以获取解决方案。
