...
在 Kotlin 中，导入是一个编译时概念——导入内容实际上不会导致任何代码运行（与 Python 不同，在 Python 中，文件中的所有顶级语句都在导入时执行）。因此，允许循环导入，但是它们可能会在代码中提示设计问题。但是，在执行期间，将在首次引用类（或其任何属性或函数）时加载类，并且类加载会导致初始化 [伴生对象](objects-and-companion-objects.html#伴生对象)——如果具有循环依赖项，则可能导致运行时异常。
