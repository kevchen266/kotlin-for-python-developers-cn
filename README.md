...
Kotlin 允许通过 _可见性修饰符_（可以放置在符号声明上）强制执行符号可见性（Python 仅通过下划线约定来实现）。如果不提供可见性修改器，则会获得默认的 _可见性_ 级别，该级别为 _public_ 。
...
* `public`（或省略）：任何可以看到包含类的代码都可以看到此符号
* `internal`：此符号仅对与声明此符号的文件所在模块属于同一模块的文件中的代码可见，并且还可以查看包含的类
* `protected`：此符号仅在包含类及其所有子类中可见，无论它们在何处声明（因此，如果类是公共的且[开放](#子类化)的，则任何人都可以对其进行子类化，从而查看并使用受保护的成员）。如果使用过 Java：这也 _不_ 会授予其余包的访问权限。
...
属性的类型以及用于参数的类型和函数的返回类型，必须与属性/函数本身“至少一样可见”。例如，公共函数不能将私有类型作为参数。

可见性级别仅影响 _符号_ 的 _词汇可见性_ ——也就是说，编译器允许键入符号。它不会影响 _实例_ 的使用位置：例如，公共顶级函数很可能会返回私有类的实例，只要返回类型没有提及私有类名称，而是它的公共基类即可。私有类（可能是 `Any`）或私有类实现的公共接口。当对某个子类进行[子类化](#子类化)时，子类也继承了它的私有成员，但是在该子类中不能直接访问它——但是，如果调用碰巧访问了私有成员的继承的公共函数，那也没关系。
