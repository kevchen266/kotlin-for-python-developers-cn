...
幸运的是，Kotlin 得到了 _具体化的类型参数_，从而缓解了其中的一些问题。通过在泛型参数前面编写 `reified`，它在运行时确实可用，并且需要编写 `T::class` 来获取[类元数据](#由类引用获取成员引用)。只能在内联函数中这样做（因为内联函数将被编译到它的调用站点中，_其_ 中类型信息在运行时可用），但它仍然需要很长时间。例如，可以为签名不太优雅的大型函数创建内联包装器函数。
